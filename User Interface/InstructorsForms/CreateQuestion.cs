    ...
    public partial class CreateQuestion : Form
    {
        ...
        private void button6_Click_1(object sender, EventArgs e)
        {
            ...
            if (flag == false)
            {
                ...
                string questionText = QuestionBody.Text.Trim();
                string questionType = selectedItem;

                string optionA = Opt1.Text.Trim();
                string optionB = Opt2.Text.Trim();
                string optionC = Opt3.Text.Trim();
                string optionD = Opt4.Text.Trim();
                string correctOption = Answer.Text.Trim();

                
                HashSet<string> options = new HashSet<string> { optionA, optionB, optionC, optionD };
                if (options.Count < 4 && questionType == "MCQ")
                {
                    MessageBox.Show("Options must be unique.");
                    return;
                }
                if (options.Count < 2 && questionType == "True&False")
                {
                    MessageBox.Show("Options must be unique.");
                    return;
                }
                ...
                {
                    MessageBox.Show("The correct answer must be one of the options.");
                    return;
                }
                ...
                QuestionDetails.OptionA = optionA;
                QuestionDetails.OptionB = optionB;
                QuestionDetails.OptionC = optionC;
                QuestionDetails.OptionD = optionD;
                QuestionDetails.CorrectOption = correctOption;
