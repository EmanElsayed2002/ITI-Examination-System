...
namespace Business_Logic_Layer.EntityManager
{
    public class OptionManager
    {
        ...
        public static int UpdateOptionToBeCorrected(string optionText)
        {
            string query = "update Options set IsCorrect = 1 where OptionText = @Answer";
            SqlParameter[] sp = {
                new SqlParameter("@Answer", optionText)
            };
            int rowsAffeted = Data_Access_Layer.DBManager.ExecuteNonQuery(query, sp);
            return rowsAffeted;
        }
